# 🌦️ Weather & News Aggregator

A modern web application that combines real-time weather updates with the latest news in one place. Users can explore weather information, browse trending news, search articles, and view detailed news pages through a clean and responsive interface.

## 🚀 Live Demo

🔗 https://weather-news-aggregator.netlify.app/

---

## ✨ Features

- 🌤️ Real-time Weather Information
- 🌞 Search Weather by City Name
- 📰 Trending News
- 🔍 Search News by Keyword
- 🏷️ Browse News by Category
- 📖 Detailed News Page
- 📱 Fully Responsive Design
- 🎨 Modern and Clean UI

---

## 🛠️ Tech Stack

- HTML
- CSS
- JavaScript
- OpenWeather API
- GNews API

---

## 📁 Project Structure

```text
Weather-News-Aggregator-LeapX-Project-3
│
├── assets/
├── cssFiles/
├── htmlFiles/
├── jsFiles/
└── README.md
```

---

## 📷 Screenshots

<img width="1470" height="798" alt="Screenshot 2026-07-07 at 2 36 07 PM" src="https://github.com/user-attachments/assets/053b7949-954d-44bd-83db-0079c956dfc4" />
<br/>
<img width="1470" height="798" alt="Screenshot 2026-07-07 at 2 36 20 PM" src="https://github.com/user-attachments/assets/81f68666-1e5c-497e-b89d-d58e716dc705" />
<br/>
<img width="1470" height="798" alt="Screenshot 2026-07-07 at 2 36 29 PM" src="https://github.com/user-attachments/assets/3e5d73d1-35a7-46ff-8402-60810e565a15" />


---

## 💡 Future Improvements

- Bookmark News
- Weather Based News Suggestions
- Location-based Weather
- Better Search Experience

---

## 🤝 Contributors

**Aman Kumar**
**Bhumika Sharma**

---
